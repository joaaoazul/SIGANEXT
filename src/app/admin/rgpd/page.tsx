...
export default function RgpdPage() {
  ...
  if (!data) {
    return <div className="text-center py-20"><p className="text-gray-400">Erro ao carregar dados RGPD.</p></div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Proteção de dados e conformidade regulatória</p>
        </div>
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-white">Score de Conformidade</h2>
              <p className="text-xs text-gray-500">{data.complianceScore.score} de {data.complianceScore.total} requisitos cumpridos</p>
            </div>
          </div>
      ...
      {tab === "compliance" && (
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
            <h3 className="text-sm font-semibold text-white flex items-center gap-2 mb-4">
              <Clock className="w-4 h-4 text-gray-400" /> Retenção de Dados
            </h3>
            <div className="space-y-3 text-xs">
              <div className="flex justify-between">
                <span className="text-gray-500">Última limpeza automática</span>
                <span className="text-gray-300 font-medium">
                  {data.dataRetention.lastCleanup ? new Date(data.dataRetention.lastCleanup).toLocaleString("pt-PT") : "Nunca executado"}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Convites pendentes</span>
                <span className="text-gray-300 font-medium">{data.invites.pending}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Convites expirados</span>
                <span className="text-gray-300 font-medium">{data.invites.expired}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <p className={`text-2xl font-bold ${data.incidents.open > 0 ? "text-orange-400" : "text-gray-400"}`}>{data.incidents.open}</p>
                <p className="text-[10px] text-gray-500 mt-0.5">Abertos</p>
              </div>
              <div>
                <p className={`text-2xl font-bold ${data.incidents.critical > 0 ? "text-red-400" : "text-gray-400"}`}>{data.incidents.critical}</p>
                <p className="text-[10px] text-gray-500 mt-0.5">Críticos</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-gray-300">{data.incidents.total}</p>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
              ...
              <div className="space-y-3 text-xs">
                {[
                  { label: "Total", value: data.consent.totalUsers, color: "text-white" },
                  { label: "Com consentimento", value: data.consent.usersWithConsent, color: "text-emerald-400" },
                  { label: "Sem consentimento", value: data.consent.usersWithoutConsent, color: data.consent.usersWithoutConsent > 0 ? "text-red-400" : "text-gray-400" },
                ...
                {data.consent.totalUsers > 0 && (
                  <div className="pt-3 border-t border-gray-800">
                    ...
                    <p className="text-[10px] text-gray-500 mt-1 text-right">{Math.round((data.consent.usersWithConsent / data.consent.totalUsers) * 100)}% com consentimento</p>
                  </div>
                )}

                <div className="pt-3 border-t border-gray-800">
                  <span className="text-[10px] text-gray-600 uppercase tracking-wider">Versão atual da política</span>
                  <p className="text-xs font-medium text-white mt-0.5">v{data.consent.currentPolicyVersion}</p>
                </div>
                  ...
                  <div className="pt-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {data.consent.consentVersions.map((v) => (
                        <span key={v.version} className="text-[10px] px-2 py-0.5 bg-gray-800 rounded-full text-gray-400">
                          v{v.version}: {v.count}
                        </span>
                      ))}
                    </div>
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
              ...
              <div className="space-y-3 text-xs">
                {[
                  { label: "Total", value: data.clients.total, color: "text-white" },
                  { label: "Ativos", value: data.clients.active, color: "text-emerald-400" },
                  { label: "Inativos", value: data.clients.inactive, color: "text-gray-400" },
                ...
                {data.clients.total > 0 && (
                  <div className="pt-3 border-t border-gray-800">
                    ...
                    <p className="text-[10px] text-gray-500 mt-1 text-right">{Math.round((data.clients.withConsent / data.clients.total) * 100)}% com consentimento</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {tab === "audit" && (
        <div className="space-y-6">
                ...
                {data.audit.actionBreakdown.map((a) => {
                  ...
                  return (
                    <div key={a.action} className="flex items-center gap-3">
                      <span className="text-xs text-gray-400 font-mono w-36 truncate">{a.action}</span>
                      <div className="flex-1 bg-gray-800 rounded-full h-2">
                        <div className="bg-blue-500/60 h-2 rounded-full transition-all" style={{ width: `${(a.count / maxCount) * 100}%` }} />
                      </div>
                      <span className="text-xs font-medium text-gray-300 w-12 text-right">{a.count}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
            <h3 className="text-sm font-semibold text-white flex items-center gap-2 mb-4">
              <FileText className="w-4 h-4 text-gray-400" /> Registos Recentes
            </h3>
            {data.audit.recentLogs.length === 0 ? (
              <p className="text-xs text-gray-500 py-4 text-center">Sem registos</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-800">
                      <th className="text-left py-2 px-2 text-gray-500 font-medium">Ação</th>
                      <th className="text-left py-2 px-2 text-gray-500 font-medium">Entidade</th>
                      <th className="text-left py-2 px-2 text-gray-500 font-medium">Utilizador</th>
                      <th className="text-left py-2 px-2 text-gray-500 font-medium">IP</th>
                      <th className="text-right py-2 px-2 text-gray-500 font-medium">Data</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.audit.recentLogs.map((log: any) => (
                      <tr key={log.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                        <td className="py-2 px-2">
                          <span className="font-mono bg-gray-800 px-1.5 py-0.5 rounded text-yellow-400">{log.action}</span>
                        </td>
                        <td className="py-2 px-2 text-gray-400">{log.entity || "—"}</td>
                        <td className="py-2 px-2 text-gray-400 max-w-[120px] truncate">{log.userEmail || "—"}</td>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-white">{data.audit.loginAttempts}</p>
                <p className="text-[10px] text-gray-500 mt-0.5">Tentativas</p>
              </div>
              <div className="text-center">
                <p className={`text-2xl font-bold ${data.audit.failedLogins > 10 ? "text-red-400" : "text-gray-300"}`}>{data.audit.failedLogins}</p>
                <p className="text-[10px] text-gray-500 mt-0.5">Falhados</p>
              </div>
              <div className="text-center">
                ...
                <p className="text-[10px] text-gray-500 mt-0.5">Taxa Sucesso</p>
              </div>
            </div>
          </div>
        </div>
