...
export default function FeedbackPage() {
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, { class: string; label: string }> = {
      pending: { class: "bg-yellow-50 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400", label: "Pendente" },
      reviewed: { class: "bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400", label: "Analisado" },
      resolved: { class: "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400", label: "Resolvido" },
    };
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {feedbacks.slice((page - 1) * 20, page * 20).map((fb) => (
            <div key={fb.id} className="card hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-white truncate">{fb.subject}</h3>
                    {statusBadge(fb.status)}
                    <span className="badge bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400">{typeLabels[fb.type] || fb.type}</span>
                  </div>
                  ...
                  {fb.rating && (
                    <div className="flex items-center gap-1 mt-2">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} className={`w-3.5 h-3.5 ${s <= fb.rating! ? "text-yellow-600 fill-yellow-400" : "text-gray-500 dark:text-gray-400"}`} />
                      ))}
                    </div>
                  )}
                  {fb.response && (
                    <div className="mt-2 p-2 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">{fb.response}</p>
                    </div>
                  )}
                </div>
                <div className="flex gap-1 shrink-0">
                  <button onClick={() => { setShowDetail(fb); setResponseText(fb.response || ""); }} className="p-2 hover:bg-white dark:hover:bg-gray-800 rounded-lg">
                    <Eye className="w-4 h-4 text-gray-500" />
                  </button>
                  {fb.status !== "resolved" && (
                    <button onClick={() => handleResolve(fb.id)} className="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg" title="Marcar como resolvido">
                      <Send className="w-4 h-4 text-emerald-600" />
                    </button>
                  )}
                  <button onClick={() => handleDelete(fb.id)} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Enviar Feedback">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Assunto *</label>
        ...
        {showDetail && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-gray-400">Cliente:</span> <span className="text-gray-900 dark:text-white ml-2">{showDetail.client.name}</span></div>
              <div><span className="text-gray-400">Tipo:</span> <span className="text-gray-900 dark:text-white ml-2">{typeLabels[showDetail.type]}</span></div>
              <div><span className="text-gray-400">Estado:</span> <span className="ml-2">{statusBadge(showDetail.status)}</span></div>
              <div><span className="text-gray-400">Data:</span> <span className="text-gray-900 dark:text-white ml-2">{new Date(showDetail.createdAt).toLocaleDateString("pt-PT")}</span></div>
            </div>
            <div>
              <p className="text-sm text-gray-400 mb-1">Mensagem:</p>
              <p className="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl">{showDetail.message}</p>
            </div>
            {showDetail.status !== "resolved" && (
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Resposta</label>
