...
async function main() {
  ...
  const clients = await Promise.all([
    prisma.client.create({
      data: {
        ...
        height: 178, weight: 82, bodyFat: 18, goal: "Perda de gordura",
        paymentStatus: "paid", paymentDueDate: new Date("2025-03-25"),
    ...
    prisma.client.create({
      data: {
        ...
        height: 165, weight: 58, bodyFat: 22, goal: "Ganho muscular",
        paymentStatus: "paid", paymentDueDate: new Date("2025-03-20"),
        managedById: admin.id,
      },
    }),
    prisma.client.create({
      data: {
        ...
        height: 185, weight: 95, bodyFat: 25, goal: "Hipertrofia",
        paymentStatus: "overdue", paymentDueDate: new Date("2025-02-15"),
        managedById: admin.id,
      },
    }),
    prisma.client.create({
      data: {
        ...
        height: 160, weight: 55, bodyFat: 20, goal: "Manutenção e saúde",
        paymentStatus: "paid", paymentDueDate: new Date("2025-03-30"),
        managedById: admin.id,
      },
    }),
    prisma.client.create({
      data: {
        ...
        height: 175, weight: 70, bodyFat: 15, goal: "Performance desportiva",
        paymentStatus: "pending", managedById: admin.id,
      },
    }),
  ]);
  ...
  await Promise.all([
    prisma.feedback.create({
      data: {
        clientId: clients[0].id, userId: admin.id,
    ...
    prisma.feedback.create({
      data: {
        clientId: clients[1].id, userId: admin.id,
  ...
  const slot1 = await prisma.bookingSlot.create({
    data: { userId: admin.id, date: tomorrow, startTime: "09:00", endTime: "10:00", maxClients: 1 },
  });
  const slot2 = await prisma.bookingSlot.create({
    data: { userId: admin.id, date: tomorrow, startTime: "10:00", endTime: "11:00", maxClients: 3, notes: "Aula de grupo" },
  });
  await prisma.bookingSlot.create({
    data: { userId: admin.id, date: tomorrow, startTime: "14:00", endTime: "15:00", maxClients: 1 },
  });

  await prisma.booking.create({
    data: { slotId: slot1.id, clientId: clients[0].id, status: "confirmed" },
  });
  await prisma.booking.create({
    data: { slotId: slot2.id, clientId: clients[1].id, status: "pending" },
  });
  ...
  await Promise.all([
    prisma.notification.create({
      data: { userId: admin.id, title: "Pagamento em atraso", message: "Ricardo Almeida tem o pagamento em atraso desde 15/02/2025.", type: "warning", link: `/clients/${clients[2].id}` },
    }),
    prisma.notification.create({
      data: { userId: admin.id, title: "Novo feedback", message: "Miguel Santos enviou um feedback sobre o treino.", type: "info", link: "/feedback" },
    }),
    prisma.notification.create({
      data: { userId: admin.id, title: "Plano de treino concluído", message: "Miguel Santos concluiu 8 semanas do plano Full Body.", type: "success" },
    }),
  ]);
  ...
  await Promise.all([
    prisma.content.create({
      data: { userId: admin.id, title: "Guia de Agachamento Perfeito", description: "Aprende a técnica correta do agachamento passo a passo.", type: "video", category: "Treino", url: "https://youtube.com/example", isPublic: true },
    }),
    prisma.content.create({
      data: { userId: admin.id, title: "eBook: Nutrição para Atletas", description: "Manual completo sobre macronutrientes e timing nutricional.", type: "ebook", category: "Nutrição", isPublic: true },
    }),
    prisma.content.create({
      data: { userId: admin.id, title: "10 Dicas para Melhor Recuperação", description: "Sono, hidratação, suplementação e mais.", type: "article", category: "Recuperação", isPublic: true },
    }),
  ]);
